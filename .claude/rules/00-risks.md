...
- 測試分兩層：一般 Vitest 覆蓋純邏輯；`npm run test:integration` 會連本機 Auth／Firestore emulator，覆蓋 Account/Transfer、訂閱自動入帳、月度 Budget、Backup Restore。**Functions／OCR 行為仍未有自動整合測試**。【實測：src/integration/storage.integration.ts＋README】
