...
✅ 正例：把 getCategoryBreakdown 改用 sumMoney——附 Vitest：同一組含浮點噪音的 20 筆假交易，新舊實作逐分類比較，差異只有噪音清理（0.30000000000000004 → 0.3），列表證明。（此例其後真的照此執行並併入 main：75cf3f9。）
...
3. **對照地雷清單**（20-repo-map §地雷）：常見來源是 #4 幣別混加、#5 預算無月份、#6 目標雙重真相；**分類聚合**的裸浮點加總已修復（storage 75cf3f9、screens 2026-07-11；殘存一處純顯示的 AnalysisScreen 日長條裸加，下游取整、影響 ≤1 元，掛在 backlog #4），同型新犯仍是頭號嫌疑——新寫聚合一律先用 financeLogic 現成 helpers。
...
- 2026-07-08 R-P4 地雷引用同步（#2/#3 已修）、R-P1 正例補後續（Fable 覆核）。
- 2026-07-11 R-P4 步驟 3 更新（screens 裸加總已修）。
